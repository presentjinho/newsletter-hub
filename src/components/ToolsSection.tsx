...
export default function ToolsSection() {
  return (
    <section className="stack bg-[#e8efe8] dark:bg-[var(--surface)] p-8 md:p-14 border-b border-line-alpha" id="stack">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {stackTools.map((tool, idx) => (
            <article
              key={idx}
              className="stack-card p-5 border border-line-alpha bg-white/55 dark:bg-[var(--surface)]/80 flex flex-col justify-between min-h-[190px] transition duration-200 hover:shadow-sm"
            >
              <div>
                <span className="inline-block px-2 py-0.5 border border-forest-green/30 text-forest-green dark:text-[var(--green)] text-[10px] font-bold uppercase mb-3">
                  {tool.kind}
                </span>
